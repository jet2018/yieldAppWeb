...
def post_a_Post(request):
	if request.method == 'POST':
		...
		if Image == '' and video == '' and body == '' and title == '':
			messages.warning(request, "Are you submitting an empty post")

		elif Image != '' and video != '':
			mypost = Post(title = title, body = body, Image = Image, posted_by = request.user)
			mypost.save()
			...
			mypost = Post(title = title, body = body, video = video, posted_by = request.user)
			mypost.save()

		else:
			mypost = Post(title = title, body = body, posted_by = request.user)
			mypost.save()
...
class PostDetailView(DetailView):
	...
	def get_context_data(self, **kwargs):
		context = super().get_context_data(**kwargs)
		context['comments'] = postComment.objects.filter(comment_to = self.object)
		return context
